<?php 
  contact_section()
?> 

   <section class="section section-follow grey darken-3 white-text center">
     <div class="container">
       <div class="row">
         <div class="col s12">
           <h4>Follow Grey Stone</h4>
           <p>Follow us on social media for our latest news and projects</p>
           <a href="https://facebook.com" target="_blank" class="white-text">
             <i class="fab fa-facebook fa-4x"></i>
           </a>
           <a href="https://Twitter.com" target="_blank" class="white-text">
             <i class="fab fa-twitter fa-4x"></i>
           </a>
           <a href="https://linkedin.com" target="_blank" class="white-text">
             <i class="fab fa-linkedin fa-4x"></i>
           </a>
           <a href="https://googleplus.com" target="_blank" class="white-text">
             <i class="fab fa-google-plus fa-4x"></i>
           </a>
           <a href="https://pinterest.com" target="_blank" class="white-text">
             <i class="fab fa-pinterest fa-4x"></i>
           </a>
         </div>
       </div>
     </div>
   </section>
   
 

  
  
  <!-- Footer -->
  <footer class="page-footer grey darken-4">
    <div class="container">
      <div class="row">
        <div class="col l6 s12">
          <h5>Location</h5>
          <p>Grey Stone</p>
          <p>18 Somewhere Drive, Ripon, HG4 1TE</p>
        </div>
        <div class="col l4 offset-l2 s12">
          <h5 class="white-text">Links</h5>
          <ul>
            <li>
              <a class="white-text" href="<?php echo esc_url(site_url('/home')) ?>">Home</a>
            </li>
            <li>
              <a class="white-text" href="<?php echo esc_url(site_url('/about')) ?>">About</a>
            </li>
            <li>
              <a class="white-text" href="<?php echo esc_url(site_url('/blog')) ?>">News</a>
            </li>
            <li>
              <a class="white-text" href="<?php echo esc_url(site_url('/contact')) ?>">Contact</a>
            </li>
          </ul>
        </div>
      </div>
    </div>
    <div class="footer-copyright grey darken-4">
      <div class="container">
        Built by Grey Stone © 2018
        <a class="grey-text text-lighten-4 right" href="#home">
          <i class="material-icons left">keyboard_arrow_up</i> Back To Top
        </a>
      </div>
    </div>
  </footer>
  <?php wp_footer(); ?>
  </body>
</html>

